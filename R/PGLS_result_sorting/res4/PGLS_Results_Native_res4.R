...
resampled1<-readRDS("pgls_results/FBD_crown_resampled_25_nonnative_concensus_res4.rds")

#read in the original data that you used to generate pgls results
pgls_data<-readRDS("pgls_results/PGLS_datasets_resample_native/pgls_data_res4_s25.rds")
...
urbanhmetrics1<-readRDS("geo_data/lanscapemetrics_native_lc_res4.rds")
...
saveRDS(diff_metrics, "native_nonnative_results/landscape_metrics_native_res4.rds")
saveRDS(resampled2, "native_nonnative_results/native_landscape_results_res4.rds")


urbanhmetrics_area<-readRDS("geo_data/lanscapemetrics_native_lc_area_res4.rds")
...
urbanhmetrics_area1<-urbanhmetrics_area %>%
  ...
  rename(area = value)
saveRDS(urbanhmetrics_area1, "native_nonnative_results/area_corrected_native_res4.rds")


metrics_GI<-readRDS("geo_data/lanscapemetrics_native_lc_GI_res4.rds")
...
wtf3 <-left_join(wtf3,metrics_GI1, by = c("ID" = "Position"))
coordinates<-hexes %>%
...
saveRDS(model_data_native, "geo_data/spatial_glm_data/native/model_all_res4.rds")
