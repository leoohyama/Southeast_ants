...
native_only_res4<-ggplot(data = full_senseres4) +
  geom_point(aes(x = model_used_no_sig, y = mean_slope_QWD)) +
  facet_wrap(~sample_size) +
  labs(x = "No. of PGLS models to calculate parameter",
        y = "Average QWD slope coefficicent", title = "~1,770 km2 for native only data") +
  theme_bw()
...
saveRDS(model_data, "difference_data/differences_res4.rds")
...
meshe <- make_mesh(data_model, xy_cols = c("X", "Y"), n_knots = 60, type = "kmeans")
...
##now assess exotic percentage against slope sample size 20
meshe <- make_mesh(data_model, xy_cols = c("X", "Y"), n_knots = 35,type = "kmeans")
