...
full_senseres4<-readRDS("pgls_results/FBD_crown_resampled_native_concensus_res3_sensitivity_data.rds")
full_senseres4_nn<-readRDS("pgls_results/FBD_crown_resampled_nonnative_concensus_res3_sensitivity_data.rds")
...
metrics_res4<-readRDS("geo_data/lc_res3.rds")
metrics_area_res4<-readRDS("geo_data/lc_area_res3.rds")
metrics_condent_res4<-readRDS("geo_data/lc_condent_res3.rds")
...
saveRDS(differences, "difference_data/differences_res3.rds")
...
saveRDS(model_data, "difference_data/differences_res3.rds")
...
meshe <- make_mesh(data_model, xy_cols = c("X", "Y"), n_knots = 10)
...
meshe <- make_mesh(data_model, xy_cols = c("X", "Y"), n_knots = 20)
...
##now assess exotic percentage against slope sample size 20
meshe <- make_mesh(data_model, xy_cols = c("X", "Y"), n_knots = 10,type = "kmeans")
